import {
  HttpMethod,
  type IDojahBalance,
  type IDojahBVNDetails,
  type IDojahNINDetails,
} from "@/common";
import { callDojah } from "./connect.dojah";

export const dojahBVNLookup = async (bvn: string) => {
  const { data, error } = await callDojah<{ entity: IDojahBVNDetails }>(
    "/kyc/bvn/full",
    HttpMethod.GET,
    { params: { bvn } },
  );
  ...
  return {
    data: {
      ...
      phoneNumber:
        item.mobile || item.phone || item.phone_number || item.phone_number1,
...
export const dojahNINLookup = async (nin: string) => {
  const { data, error } = await callDojah<{ entity: IDojahNINDetails }>(
    "/kyc/nin/nin_slip",
    HttpMethod.GET,
    { params: { nin } },
  );
...
export const dojahBalance = async () => {
  const { data, error } = await callDojah<IDojahBalance>(
    "/balance",
    HttpMethod.GET,
  );
